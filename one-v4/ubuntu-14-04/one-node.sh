...
#--------------------------------------------------
echo -e "\n---- Install latest updates ----"
sudo apt-get update && apt-get dist-upgrade -y && apt-get autoremove
...
sudo echo "deb http://downloads.opennebula.org/repo/4.12/Ubuntu/14.04/ stable opennebula" > \ /etc/apt/sources.list.d/opennebula.list
sudo apt-get update
...
# The original 'interfaces' file has been renamed to 'interfaces.bkp'
...
echo "Done! The OpenNebula node is ready now. Add the host '$HOSTNAME' via Sunstone or CLI on ONE-Server ($ONE_SERVERIP) now"